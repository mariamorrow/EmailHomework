...
class RootTVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            print("delete")
            let deletedEmail = self.emails[indexPath.row]
            emails.remove(at: indexPath.row)
            tableView.deleteRows(at: [indexPath], with: .fade)
            editdelegate?.editEmail(action: "delete", context: "Trash", email: deletedEmail)
             }
        else if editingStyle == .insert {
            let test = Email(sender: "asu@asu.edu", subject: "Spam", contents: "Spam")
            self.emails.append(test)
            
            // Create a new instance of the appropriate class, insert it into the array, and add a new row to the table view
            
            //
            // editdelegate?.editEmail(action: "add", context: "add", email: test)
    }
